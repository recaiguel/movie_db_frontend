...
$stmt = $pdo->prepare("SELECT * FROM regie WHERE id = ?");
$stmt->execute([$id]);
...
<div class="main-content">
    <h2>Regisseur bearbeiten</h2>
        ...
        <div id="success-popup">
            Regisseur erfolgreich geändert!
        </div>
    <?php endif; ?>

    <form method="POST" action="" autocomplete="off">
        <p><strong>Vorname:</strong></p>
        <input type="text" name="vorname" value="<?= htmlspecialchars($director['vorname']) ?>" required>

        <p><strong>Nachname:</strong></p>
        <input type="text" name="nachname" value="<?= htmlspecialchars($director['nachname']) ?>" required>

        <p><strong>Geburtsdatum:</strong></p>
        <input type="date" name="geburtsdatum" value="<?= htmlspecialchars($director['geburtsdatum']) ?>">

        <p><strong>Geschlecht:</strong></p>
        <select name="geschlecht">
            <option value="">-- Bitte wählen --</option>
            <option value="männlich" <?= $director['geschlecht'] === 'männlich' ? 'selected' : '' ?>>Männlich</option>
            <option value="weiblich" <?= $director['geschlecht'] === 'weiblich' ? 'selected' : '' ?>>Weiblich</option>
        </select>
        ...
        <select name="nationalitaet">
            ...
            foreach ($countries as $country): ?>
                <option value="<?= $country ?>" <?= $director['nationalität'] === $country ? 'selected' : '' ?>><?= $country ?></option>
            <?php endforeach; ?>
        </select>
